...
class StateTest extends FunSuite {
  test("testNextInt") {
    val seed = 42
    val expected = (16159453, Simple(1059025964525L))
    val actual = Simple(42).nextInt
  ...
  test("testDouble") {
    val rng = Simple(Simple(42).nextInt._1)
  ...
  test("testSimulateMachine from book case") {
    ...
    val (_, actual) = State.simulateMachine(inputs).run(machine)
    assert(actual == expected)
  }
